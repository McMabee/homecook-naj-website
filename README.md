...
Marketing site for Home Cooking with Naj, a private chef and culinary creator based in Ancaster, Ontario, serving Hamilton, Burlington, and Oakville. Views: Home (hero, about, services, Instagram tiles, service area), Recipes, Contact, and a hidden admin for recipes, Instagram tiles, and social links.
...
4. **Photos.** `src/assets/` holds web-sized copies of the four photos in `pics/` (the originals, which are not deployed). The hero and About section use two of them. Stock Unsplash photos are still used for the service cards, the "every table deserves a little magic" band, the Recipes and Contact page headers, and the three seed recipes. Swap any of them by dropping a file into `src/assets/` and updating the `src` in the relevant component.
